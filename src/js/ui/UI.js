export class UI {
	...
	draw(context) {
		...
		// Level 1: hearts collected / 5; time_attack: time left + score; until_lose: score / target; others: score and time
		...
		if (this.game.levelConfig?.heartsToCollectToWin != null && !mode) {
			...
			const timeStr =
				timeLeftSec >= 60
					? Math.floor(timeLeftSec / 60) + '.' + (timeLeftSec % 60).toFixed(1)
			...
			if (this.game.win) {
				...
				if (this.game.levelConfig?.mode === 'time_attack') {
					this.gameOverMessage.textContent =
						'Time is up! Score: ' + this.game.score;
				} else if (this.game.levelConfig?.mode === 'until_lose') {
					this.gameOverMessage.textContent =
						'You reached the target! Score: ' + this.game.score;
